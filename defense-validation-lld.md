...
```mermaid
flowchart LR
    O[Janus orchestration] -->|Submit + callback headers| A[Defense-validation API]
...
`POST /v1/defense-validation-runs` requires `Idempotency-Key`,
`X-Correlation-ID`, and an `application/json` body satisfying
`defense-validation@1.0`.
...
```text
defense-validation:<run_id>:terminal:v1
```

The callback columns on `defense_validation_run` are the transactional outbox
...
```json
{
  ...
  "wakeup": {
    "event_id": "defense-validation:<run-id>:terminal:v1",
    "capability": "defense-validation",
    "request_id": "<defense-validation-request-id>",
    "correlation_id": "<correlation-id>",
    "run_id": "<defense-validation-run-id>"
  }
}
```
...
PostgreSQL integration tests use `DV_TEST_DATABASE_URL`; callback HTTP tests use
...
| Callback event ID | `defense-validation:<run-id>:terminal:v1` |
