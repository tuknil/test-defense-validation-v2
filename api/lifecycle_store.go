...
func migrateLifecycle(db *sql.DB) error {
	statements := []string{
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS request_id TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS correlation_id TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS request_digest TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS status TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS started_at TIMESTAMPTZ`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS progress_phase TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS progress_message TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS failure JSONB`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS completion JSONB`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS worker_id TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS lease_token TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS lease_expires_at TIMESTAMPTZ`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS attempt INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS last_heartbeat TIMESTAMPTZ`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS cancel_requested BOOLEAN NOT NULL DEFAULT FALSE`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS publication_pending BOOLEAN NOT NULL DEFAULT FALSE`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS result_payload BYTEA`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS verification_attempt INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS publication_retry_at TIMESTAMPTZ`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_url TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_workflow_id TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_signal TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_event_id TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_state TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_attempt INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_next_at TIMESTAMPTZ`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_delivered_at TIMESTAMPTZ`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_configuration_failed_at TIMESTAMPTZ`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_last_error TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_lease_token TEXT`,
		`ALTER TABLE defense_validation_run ADD COLUMN IF NOT EXISTS callback_lease_expires_at TIMESTAMPTZ`,
		`CREATE UNIQUE INDEX IF NOT EXISTS defense_validation_run_request_id_uq ON defense_validation_run(request_id) WHERE request_id IS NOT NULL`,
		`CREATE INDEX IF NOT EXISTS defense_validation_run_worker_idx ON defense_validation_run(status, lease_expires_at, created_at)`,
		`CREATE INDEX IF NOT EXISTS defense_validation_run_callback_idx ON defense_validation_run(callback_state, callback_next_at) WHERE callback_url IS NOT NULL`,
		`UPDATE defense_validation_run SET status=CASE WHEN terminal_state='malfunction' THEN 'failed' ELSE 'completed' END,
		...
		`UPDATE defense_validation_run SET callback_state=CASE
...
func backfillCanonicalResultPayloads(db *sql.DB) error {
	...
	rows, err := tx.Query(`SELECT run_id,status,publication_pending,response FROM defense_validation_run
	...
	for _, item := range pending {
		if _, err := tx.Exec(`UPDATE defense_validation_run SET response=$2::jsonb,result_payload=$3
...
func getDurable(ctx context.Context, q rowQuerier, column, value string) (DurableRun, error) {
	query := `SELECT run_id,request_id,correlation_id,request_digest,status,NULLIF(terminal_state,''),
		...
		COALESCE(callback_workflow_id,''),COALESCE(callback_signal,''),COALESCE(callback_event_id,'') FROM defense_validation_run WHERE ` + column + `=$1`
	var run DurableRun
	...
	run.Capability = "defense-validation"
...
func (r *DurableRun) setCompletion() {
	...
	o := r.Result
	r.Completion = &Completion{Capability: "defense-validation", ContractID: "capability-completion@1.0",
...
func (s *RunStore) GetStatus(ctx context.Context, id string) (RunStatus, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT run_id,request_id,correlation_id,status,
		...
		FROM defense_validation_run WHERE run_id=$1`, id).Scan(&status.RunID, &status.RequestID,
	...
	status.Capability = "defense-validation"
...
func (s *RunStore) LeaseNext(ctx context.Context, workerID string, lease time.Duration, maxAttempts int) (DurableRun, bool, error) {
	...
	defer func() { _ = tx.Rollback() }()
	var id string
	err = tx.QueryRowContext(ctx, `SELECT run_id FROM defense_validation_run WHERE
	...
	_, err = tx.ExecContext(ctx, `UPDATE defense_validation_run SET status='running',started_at=COALESCE(started_at,now()),
		...
		progress_message=CASE WHEN response='{}'::jsonb THEN 'Defense validation in progress' ELSE 'Publishing immutable result' END
...
func (s *RunStore) Heartbeat(ctx context.Context, id, workerID, leaseToken string, lease time.Duration) (bool, bool, error) {
	var cancel bool
	err := s.db.QueryRowContext(ctx, `UPDATE defense_validation_run SET last_heartbeat=now(),
...
func (s *RunStore) UpdateProgress(ctx context.Context, id, workerID, leaseToken, phase, message string) (bool, error) {
	res, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET progress_phase=$4,
...
func (s *RunStore) CompletePublished(ctx context.Context, id, workerID, leaseToken string, out RunOutcome, payload []byte) (bool, error) {
	...
	completion, err := json.Marshal(Completion{Capability: "defense-validation", ContractID: "capability-completion@1.0",
...
func (s *RunStore) FailExhausted(ctx context.Context, max int) error {
	...
	_, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET status='failed',terminal_state='malfunction',failure=$2,
...
func (s *RunStore) RecoverCanceled(ctx context.Context) error {
	_, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET status='canceled',terminal_state='canceled',
...
func (s *RunStore) Cancel(ctx context.Context, id string) (DurableRun, error) {
	_, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET cancel_requested=TRUE,
...
func (s *RunStore) MarkCanceled(ctx context.Context, id, workerID, leaseToken string) (bool, error) {
	res, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET status='canceled',terminal_state='canceled',
...
func (s *RunStore) MarkPublicationPending(ctx context.Context, id, workerID, leaseToken string) (bool, error) {
	res, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET publication_pending=TRUE,
...
func (s *RunStore) FailPublicationConflict(ctx context.Context, id, workerID, leaseToken string, f RunFailure) (bool, error) {
	failure, _ := json.Marshal(f)
	res, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET status='failed',terminal_state='malfunction',
...
func (s *RunStore) ClaimNextCallback(ctx context.Context, lease time.Duration) (CallbackDelivery, bool, error) {
	...
	var id string
	var attempt int
	err = tx.QueryRowContext(ctx, `SELECT run_id,callback_attempt FROM defense_validation_run
	...
	leaseToken := newID()
	if _, err := tx.ExecContext(ctx, `UPDATE defense_validation_run SET callback_state='delivering',
...
func (s *RunStore) CallbackSucceeded(ctx context.Context, id, leaseToken string) error {
	res, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET callback_state='delivered',callback_delivered_at=now(),
...
func (s *RunStore) CallbackFailed(ctx context.Context, id, leaseToken string, delay time.Duration, reason string) error {
	res, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET callback_state='retry',callback_attempt=callback_attempt+1,
...
func (s *RunStore) CallbackConfigurationFailed(ctx context.Context, id, leaseToken, reason string) error {
	res, err := s.db.ExecContext(ctx, `UPDATE defense_validation_run SET callback_state='configuration_failed',
